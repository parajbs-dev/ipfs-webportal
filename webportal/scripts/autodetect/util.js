...
export async function postRawCar({ url, fileIterator, signal }) {
  const headers = new Headers();

  headers.set("Content-Type", "application/car");
...
export async function postRawBody({ url, fileIterator, signal }) {
  const headers = new Headers();
...
export async function getSizeFromURL({ url, fetch = globalThis.fetch, signal }) {
  const response = await fetch(url, {
    method: "HEAD",
    signal,
  });
...
export async function* getFromURL({ url, start, end, format, signal, fetch = globalThis.fetch }) {
  const headers = new Headers();
...
export async function* getFromGateway({ url, start, end, format, signal, gatewayURL = detectDefaultGateway() }) {
